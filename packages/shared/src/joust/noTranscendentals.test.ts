 ...
 * different bits for the same input. JOUST ships the server's track rather than re-simulating on
 * the display, so this is belt-and-braces here — but the aim arrives as a vector precisely so the
 * integrator never needs an angle, and this keeps it that way.
 *
