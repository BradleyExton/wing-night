...
// =============================================================================
// Display marquee — the TV's top row. DRAWING built it (DESIGN.md §2.5, "grand
// bulb marquee") and five surfaces copied it: JOUST, FAPPY, SCHLONIC, GEO and
// EMOJI_CHARADES. These two strings were byte-identical in all six before they
// moved here (EMOJI_CHARADES only spelled the keys `teamName` / `showTitle`).
//
// No colour substitution was needed to move them, which is the opposite of
// what `RunningTotals` hit: its three copies were written in JOUST's private
// desert hexes and had to be mapped onto house tokens one for one. The
// marquee's two text styles were already pure house tokens — `text-text` and
// `text-gold` — so what moved here is byte-for-byte what each game had.
//
// The marquee's CONTAINER is deliberately NOT here. Its six copies are not
// byte-identical: three carry `py-[clamp(0.7rem,1.2vh,1.2rem)]` and three
// `py-[clamp(0.8rem,1.4vh,1.3rem)]`, and DRAWING's background is
// `from-surfaceAlt to-surface` with no shadow against the other five's brown
// gradient and inset glow (DESIGN.md §2.5 dropped DRAWING's brown on
// 2026-09-21, two hours after GEO and EMOJI_CHARADES had copied it). Sharing
// it would mean passing the padding and the background in as a class string —
// the `surfaceClassName` prop docs/takeover-layout-api.md §8 already rejected
// for the arena frame. Each game keeps its own container in its own styles.ts.
// =============================================================================
...
// The dotted bulb ring, inset inside the marquee's gold border. DRAWING, GEO
// and EMOJI_CHARADES drew it; JOUST, FAPPY and SCHLONIC did not. That was an
// omission and not a plainer-by-design choice: all three kept the `relative`
// on their marquee container that exists only to position this absolute ring,
// and none of the three renders anything else absolute inside it. Restored to
// all six at T5.2.
