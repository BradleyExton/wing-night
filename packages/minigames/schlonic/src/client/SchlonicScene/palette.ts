// Scene materials for the zone (DESIGN.md §2.11): a bright green-hill morning, deliberately
// nothing like JOUST's dusk desert or FAPPY's corridor, because SCHLONIC is the one minigame
// that is supposed to look like a 16-bit platformer and the room should know which game it is
// looking at from the sofa before a word is read.
//
...
export const schlonicPalette = {
  ...
  ring: "#ffc300",
  ringCore: "#fff3bf",
  // The schlong, in JOUST and FAPPY's own bubblegum: the one hue on a green hill that is
  // neither its turf nor its sky, so a row of them reads from the sofa.
  schlong: "#f9a3bc",
  schlongDark: "#8e2a52",
  schlongLight: "#ffe6ee",
