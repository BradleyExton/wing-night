...
test("maps each phase to the expected next phase", () => {
  ...
  assert.equal(getNextPhase(Phase.INTRO, 0, 3), Phase.MINIGAME_INTRO);
  ...
  assert.equal(getNextPhase(Phase.MINIGAME_PLAY, 1, 3), Phase.TURN_RESULTS);
  assert.equal(getNextPhase(Phase.TURN_RESULTS, 1, 3), Phase.ROUND_RESULTS);
});

test("returns MINIGAME_INTRO after round results when additional rounds remain", () => {
  assert.equal(getNextPhase(Phase.ROUND_RESULTS, 1, 3), Phase.MINIGAME_INTRO);
  assert.equal(getNextPhase(Phase.ROUND_RESULTS, 2, 3), Phase.MINIGAME_INTRO);
