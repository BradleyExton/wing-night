...
## Goal
Layer a ticket-detail view onto the WN-23 board: `/dev/board/<id>` renders one ticket's frontmatter
...
- [ ] The endpoint lives on the same dev-only router as WN-23's `/api/dev/board` — absent when
      `NODE_ENV === "production"` (covered by the existing WN-23 gating test extended, or a sibling
      assertion).
- [ ] `resolveClientRoute` resolves `/dev/board/<id>` to a detail route with an id extractor,
      ...
      the bare prefix and deeper paths); WN-23's bare `/dev/board` continues to resolve to `BOARD`
      (regression-asserted).
...
- [ ] Board cards from WN-23 link to their detail pages, and the detail page links back to
      `/dev/board`.
...
## Plan
Split out of the WN-23 grill (2026-08-14) to keep each slice one-context-sized — the user chose
...
- The detail endpoint deliberately does NOT go through the work CLI: it serves file content, not
  ...
  pickability computation — see WN-23's Plan).
...
## Links
- WN-23 (the board shell this layers on; mounting shape + server-URL resolution decided there)
