...
- 2026-08-15T21:06:49.461Z HUMAN PICK (2026-08-15, Bradley): Variant A · Sidestage, and the DRUMETTE. Recorded post-land — this is the answer the lab was built to collect, not a reopening.

PROJECTILE — RESOLVED, no follow-up needed. Drumette. It reads correctly under the current position-only integrator, so NO angular velocity is required and no ticket is minted against the WN-23 module. The flat wing bone is rejected. This closes the AC-5 open question the Plan flagged for WN-15's scope: WN-15 does NOT need to carry rotation.

DIRECTION — PICKED WITH A STRUCTURAL REVISION. Variant A is the direction, but the thrower's placement is revised: they sit at a TABLE eating wings, positioned much HIGHER than the field, and throw the wing BEHIND them over the shoulder — so the contraption sits behind and below the thrower rather than laid out in front of them. The landed lab has the thrower at floor level facing the field and throwing forward across it, so the picked picture is NOT what /dev/lab/contraption-ui currently renders. A follow-up re-cuts variant A to the revised geometry so WN-24 designs level geometry against something that has actually been looked at, rather than against this prose.

Also noted for later, deliberately NOT actioned now: the user intends to iterate on UI quality long-term. That is polish against a settled picture and belongs at port time (WN-15 / port-variant), not in the throwaway lab.
